...
# * @LastEditTime: 2023-05-06 10:33:38
# * @FilePath: /Arithmetic/Logic/MultiQuestionsMaker.py
# * @Description:掛け算
# *********************************************
import functools
...
class MultiQuestionsMaker(AbsQuestionMaker):
    ...
    def makeQuestions(self, *args, **kwargs) -> None:
        ...
        while True:
            ...
            if self.resRange[0] < result < self.resRange[1]:
                # 当クラスは全部「×」
                lstOpr = [OPR.MULTI.value] * (len(lstNum) - 1)
                # 数式の作成
                formula = self.getFormula(numList=lstNum,
                                          oprList=lstOpr)
...
if __name__ == "__main__":
    ...
    ins = MultiQuestionsMaker()
    ins.setParams(**params)
    ins.makeQuestions(digits=[1, 1])
    print(ins.answers)